...
class CollaborativeFilteringTask:
    def __init__(self,
        ...
        print("Starting CollaborativeFilteringTask")
    ...
    # FIXME: we should make a base class for this
    ...
    def launch_ps(self, ip):
		...
		'"nohup ./parameter_server --config config_cf.txt --nworkers 10000 --rank 1 &> ps_output &"'
        print("cmd:", cmd)
    ...
    def launch_lambda(self, num_workers, timeout=50):
        ...
        def error_task():
            ...
            cmd = 'ssh -o "StrictHostKeyChecking no" -i %s %s@%s ' \
                    % (self.key_path, self.ps_username, self.ps_ip_public) + \
    		  '"./parameter_server --config config_cf.txt --nworkers 10 --rank 2 --ps_ip \"%s\"" > error_out &' \
                  % self.ps_ip_private
            print('cmd', cmd)
            os.system(cmd)
    ...
    def define_config(self, ip):
        ...
        stdin, stdout, stderr = client.exec_command('echo "%s" > config_cf.txt' % config)
        print stdout.read()
        stdin, stdout, stderr = client.exec_command('cat config_cf.txt')
        print stdout.read()
        client.close()
